...
export default function PosTerminal() {
  ...
  if (isLoadingShift)
    return <div className="p-12 text-center">Loading POS...</div>;
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {products?.map((p) =>
              p.variations?.map((v: any) => (
                ...
                >
                  <div className="font-semibold text-sm mb-1 line-clamp-2 h-10">
                    {p.name}
                  </div>
                ...
                >
                  <div className="flex-1 pr-2">
                    ...
                    <div className="text-xs mt-1 text-primary">
                      {item.quantity} {item.uom_abbreviation} x $
                    ...
                    {item.applied_rules && item.applied_rules.length > 0 && (
                      <div className="text-[10px] text-green-600 mt-1 flex flex-wrap gap-1">
                        {item.applied_rules.map((r, i) => (
                          <span key={i} className="bg-green-100 px-1 rounded">
                            {r}
                          </span>
                        ))}
                      </div>
