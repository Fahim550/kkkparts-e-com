...
const emptyForm = {
  ...
  dealer_original_price: null as number | null,
  category: "running",
  image: "",
  images: [] as string[],
  sku: "",
...
const ProductsManager = () => {
    ...
    const matchedProduct = viewId
      ...
      : products.find((p) => {
          ...
          const matchesSize = productSize
            ? (p.sizes || []).some(
                (size) => String(size) === String(productSize),
              )
            : true;
  ...
  const openEdit = (p: DbProduct) => {
    ...
    setForm({
      ...
      dealer_original_price: p.dealer_original_price
      ...
      is_offer: (p as any).is_offer ?? false,
    });
    setDealerDiscountPct(
      p.dealer_price && p.price
        ? String(
            Math.round((1 - Number(p.dealer_price) / Number(p.price)) * 100),
          )
        : "",
    );
    setShowForm(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const sizesFromInput = form.sizes
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    const colorsFromInput = form.colors
      .split(",")
      .map((c) => c.trim())
      .filter(Boolean);
    // Merge input values with variation rows so manual variation sizes/colors aren't lost
    const sizesFromVariations = Array.from(
      new Set(variations.map((v) => String(v.size)).filter(Boolean)),
    );
    const colorsFromVariations = Array.from(
      new Set(variations.map((v) => String(v.color)).filter(Boolean)),
    );
    const sizesFinal = Array.from(
      new Set([...sizesFromInput, ...sizesFromVariations]),
    );
    const colorsFinal = Array.from(
      new Set([...colorsFromInput, ...colorsFromVariations]),
    );
    ...
    const data: any = {
      ...
      dealer_original_price: form.dealer_original_price,
      category: form.category,
      image: form.image || "/assets/shoe-runner-1.jpg",
      images:
        form.images.length > 0
          ? form.images
          : [form.image || "/assets/shoe-runner-1.jpg"],
      sku: form.sku,
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          onClick={openAdd}
          className="flex items-center gap-2 bg-primary text-primary-foreground px-5 py-2.5 rounded-md font-body text-sm font-bold tracking-wider uppercase hover:bg-primary/90 transition-all"
        >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left p-4 font-heading text-xs uppercase tracking-wider text-muted-foreground">
                  Product
                </th>
                <th className="text-left p-4 font-heading text-xs uppercase tracking-wider text-muted-foreground hidden md:table-cell">
                  SKU
                </th>
                <th className="text-left p-4 font-heading text-xs uppercase tracking-wider text-muted-foreground">
                  Price
                ...
                <th className="text-right p-4 font-heading text-xs uppercase tracking-wider text-muted-foreground">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((p) => (
                ...
                >
                  ...
                  <td className="p-4">
                    <p className="font-body text-sm font-bold text-primary flex items-center gap-1">
                      <DirhamIcon /> {Number(p.price)}
                    </p>
                    {p.original_price && (
                      <p className="font-body text-xs text-muted-foreground line-through flex items-center gap-1">
                        <DirhamIcon /> {Number(p.original_price)}
                      </p>
                    )}
                  </td>
                  <td className="p-4 font-body text-sm capitalize text-foreground hidden md:table-cell">
                    {p.category}
                  </td>
                  ...
                  <td className="p-4">
                    <span
                      className={`px-2 py-1 text-xs font-body font-bold rounded-full uppercase ${p.is_active ? "bg-green-500/10 text-green-600" : "bg-red-500/10 text-red-600"}`}
                    >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="bg-secondary/30 border border-border rounded-lg p-5 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-4 border border-border/50 bg-background/50 p-4 rounded-md">
                    <h4 className="font-heading text-xs uppercase tracking-wider font-bold text-foreground mb-2 pb-2 border-b border-border/50">
                      Single Customer
                    </h4>
                    <div>
                      <label className="block font-body text-xs uppercase tracking-wider text-muted-foreground mb-1 flex items-center gap-1">
                        Selling Price (<DirhamIcon className="w-3 h-3" />) *
                      </label>
                      <Input
                        type="number"
                        step="0.01"
                        ...
                        onChange={(e) => {
                          ...
                          if (dealerDiscountPct && newPrice) {
                            ...
                            newForm.dealer_price = Number(
                              (newPrice - (newPrice * pct) / 100).toFixed(2),
                            );
                          }
                          setForm(newForm);
                        }}
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g. 15 (will show as crossed out)"
                      />
                      <p className="font-body text-xs text-muted-foreground mt-1">
                        Set this higher than selling price to show a sale
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4 border border-border/50 bg-background/50 p-4 rounded-md">
                    ...
                    <div>
                      <label className="block font-body text-xs uppercase tracking-wider text-muted-foreground mb-1 flex items-center gap-1">
                        Dealer Selling Price (<DirhamIcon className="w-3 h-3" />
                        )
                      </label>
                      <Input
                        type="number"
                        ...
                        onChange={(e) => {
                          ...
                          if (dp && form.price) {
                            setDealerDiscountPct(
                              String(Math.round((1 - dp / form.price) * 100)),
                            );
                          } else {
                            setDealerDiscountPct("");
                          }
                        ...
                        onChange={(e) => {
                          ...
                          if (e.target.value && form.price) {
                            ...
                            const newDealerPrice =
                              form.price - (form.price * pct) / 100;
                            setForm({
                              ...form,
                              dealer_price: Number(newDealerPrice.toFixed(2)),
                            });
                    ...
                    <div>
                      <label className="block font-body text-xs uppercase tracking-wider text-muted-foreground mb-1 flex items-center gap-1">
                        Dealer Old Price / Strikethrough (
                        <DirhamIcon className="w-3 h-3" />)
                      </label>
                      <Input
                        type="number"
                        step="0.01"
                        value={form.dealer_original_price || ""}
                        onChange={(e) =>
                          setForm({
                            ...form,
                            dealer_original_price: e.target.value
                              ? +e.target.value
                              : null,
                          })
                        }
                        placeholder="Old price for dealers"
                      />
                      <p className="font-body text-xs text-muted-foreground mt-1">
                        If empty, falls back to Single Customer Old Price
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {variations.length > 0 ? (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-border">
                          <th className="text-left py-2 px-2 font-body text-xs uppercase tracking-wider text-muted-foreground">
                            Size
                          </th>
                          <th className="text-left py-2 px-2 font-body text-xs uppercase tracking-wider text-muted-foreground">
                            Color
                          ...
                          >
                            <td className="py-2 px-2">
                              <Input
                                value={v.size}
                                onChange={(e) =>
                                  updateVariation(i, "size", e.target.value)
                                }
                  ...
                  <p className="font-body text-xs text-muted-foreground">
                    No variations. Click "Generate" after adding sizes and
                    colors, or add them manually.
                  </p>
                )}
                <button
                  type="button"
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="button"
                  onClick={() => setShowForm(false)}
                  className="px-8 py-3 border border-border rounded-md font-body text-sm font-medium hover:bg-secondary transition-colors text-foreground"
                >
                  Cancel
                </button>
              </div>
